...
public class SubjectVMService(IDbContextFactory<ApplicationDbContext> dbContextFactory)
{
    ...
    public async Task<Result> AddSubject(SubjectVM subjectVM)
    {
        ...
        try
        {
            ...
            if (saveResult <= 0)
                return Result.Error($"{Messages.ObjectCannotBeSaved<Subject>()}: {subject.Name}");
            return Result.OK($"{Messages.ObjectSaved<Subject>()}:{subject.Name}");
        }
...
//UPDATE
    public async Task<Result> UpdateSubject(SubjectVM subject)
    {
        ...
        if (exist is null) return Result.Error($"{Messages.ObjectNotFound<Subject>()}: {subject.Name}");
        ...
        {
            return Result.Error($"{Messages.ObjectCannotBeSaved<Subject>()}: {ex.Message}");
        }
        catch (Exception ex)
        {
            return Result.Error($"{Messages.ObjectCannotBeSaved<Subject>()}: {ex.Message}");
    ...
    public async Task<Result> DeleteSubject(string subjectId)
    {
        ...
        if (exist is null) return Result.Error($"{Messages.ObjectNotFound<Subject>()}: {subjectId}");

        var relatedDocsNames = string.Join(",", exist.Docs.Select(x => x.Title));

        if (exist.Docs.Any())
            return Result.Error($"{Messages.ObjectCannotBeDeletedHasRelatedEntities<Subject>()}: {subjectId}\n" +
        ...
        try
        {
            if (await db.SaveChangesAsync() <= 0)
                return Result.Error($"{Messages.ObjectCannotBeDeleted<Subject>()}: {subjectId}");
