...
public partial class DocsPage : IDisposable
{
    ...
    string? searchString;
    HashSet<TagVM> allTagsFromDocuments = [];
    HashSet<string> selectedTagsNames = [];
    ...
    protected override async Task OnInitializedAsync()
    {
        ...
        await GetDocs();
        allTagsFromDocuments = await GetAllTaskFromExistingDocuments();
    }
    ...
    async Task GetDocs(string? subjectId = "")
    {
        ...
        if (selectedTagsNames is { Count: > 0 }) filters.Add(x => x.Tags.Any(x => selectedTagsNames.Contains(x.Name)));
    ...
    [
        x => x.Categories, x => x.Subjects, x => x.Links, x => x.Tags
    ]
    )).Data;
...
// TAGS
Task<HashSet<TagVM>> GetAllTaskFromExistingDocuments()
{
    var allTags = docs.Where(x => x.Tags is not null)
        .SelectMany(x => x.Tags)
        .DistinctBy(x=>x.Name)
        .ToHashSet();
...
async Task OnTagsSelected(IReadOnlyCollection<string> selectedTags)
{
    this.selectedTagsNames = selectedTags.ToHashSet();
    await GetDocs();
}
